create or replace package Transactions as
	...
	amoun in accTable_dhk.amount%TYPE,
	recano in accTable_dhk.anum%TYPE);
...
create or replace package body Transactions as
	...
	begin
		if ano > 4000 and ano < 5000 then
			select count(anum) into n from accTable_dhk where anum = ano;
			m := 2;
		...
		if n>0 then
			if m = 1 then
				...
				select amount, withnum into now, num from accTable_dhk where anum = ano;
			elsif m = 3 then
			...
			if amoun > now-500 then
				...
				if m = 1 then
					...
					update accTable_dhk set amount = now where anum = ano;
				elsif m = 3 then
	...
	begin
		...
		if n>0 then
			if m = 1 then
				...
				select amount into now from accTable_dhk where anum = ano;
			elsif m = 3 then
	...
	begin
		if senano > 4000 and senano <5000 then
			senderSite := 2;
			select count(anum) into n from accTable_dhk where anum = senano;
		...
		if recano > 4000 and recano <5000 then
			receiverSite := 2;
			select count(anum) into m from accTable_dhk where anum = recano;
		...
		if n=0 then
			...
			raise norecuser;
				...
				if senderSite = 1 then
					...
					select amount into nowsen from accTable_dhk where anum = senano;
				...
				if receiverSite = 1 then
					...
					select amount into nowrec from accTable_dhk where anum = recano;
			...
			if amoun > nowsen-500 then
				...
				if senderSite = 1 then
					...
					update accTable_dhk set amount = nowsen where anum = senano;
				elsif senderSite = 3 then
				...
				if receiverSite = 1 then
					...
					update accTable_dhk set amount = nowrec where anum = recano;
				elsif receiverSite = 3 then
